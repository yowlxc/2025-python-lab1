...
def sum_of_2(nums, target):
    if int(len(num)) < 2: return None 
    for x in nums: if int(x) != x: return None 
...
class TestMath(unittest.TestCase):
    ...
    def test_2_positive(self):
        self.assertEqual(sum_of_2([7, 3, 2, 56], 48), None)             # нет подходящих чисел в массиве

    def test_3_positive(self):
        self.assertEqual(sum_of_2([5], 6), None)                        # недостаточно элементов в массиве 

    def test_4_positive(self):
        self.assertEqual(sum_of_2([3, 3.5], 6), None)                   # в массиве есть не int-овые элементы (float)
